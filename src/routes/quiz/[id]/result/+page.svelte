<script lang="ts">
    ...
    function formatTime(seconds: number): string {
        ...
        const secs = Math.floor(seconds % 60); // Use Math.floor for clean seconds
    ...
    <div class="relative z-10 min-h-screen py-8 px-4">
        ...
        {:else if hasResults}
            ...
            <div class="max-w-6xl mx-auto space-y-8">
                <!-- Main Card: Header, Summary, and Details -->
                    ...
                    <div class="p-8">
                        ...
                        <div class="mb-10 bg-white/5 backdrop-blur-sm rounded-2xl p-6 border border-white/10">
                            <h2 class="font-semibold text-white mb-4 text-center text-lg">
                                {isAiExam ? 'Completion Rate' : 'ความแม่นยำ'}: 
                                <span class="text-blue-400">{percentage}%</span>
                            </h2>
                            <div class="w-full bg-white/10 rounded-full h-6 overflow-hidden backdrop-blur-sm">
                                <div class="bg-gradient-to-r from-green-400 to-emerald-500 h-full transition-all duration-1000 ease-out rounded-full shadow-lg" style="width: {percentage}%"></div>
                            </div>
