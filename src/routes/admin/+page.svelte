<script lang="ts">
    ...
    // --- START: Category Form Data ---
    let categoryForm = {
        name: '',
        description: '',
        english_name: ''
    };
    // --- END: Category Form Data ---
    ...
    async function saveCategory() {
        try {
            ...
            error = err.message;
            toastStore.error(err.message || 'Failed to save category');
        }
    }
...
<div class="min-h-screen bg-gradient-to-br from-slate-900 via-blue-950 to-indigo-950 relative overflow-hidden">
    ...
    {#if sidebarOpen}
        <div 
            ...
            on:keydown={(e) => (e.key === 'Enter' || e.key === ' ') && (sidebarOpen = false)}
        ></div>
    {/if}
        ...
        <div class="relative bg-slate-800/95 backdrop-blur-lg border border-gray-700/50 rounded-2xl shadow-2xl w-full max-w-md transform transition-all duration-300 scale-100">
            <div class="p-6">
                <div class="flex items-center justify-between mb-6">
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-400">Manage exam category information</p>
                        </div>
                    </div>
                    <button
                ...
                <form on:submit|preventDefault={saveCategory} class="space-y-4">
                    <div>
                        <label for="category_name" class="block text-sm font-medium text-gray-300 mb-2">Category Name (Thai)</label>
                        <input
                            id="category_name"
                            type="text"
                            bind:value={categoryForm.name}
                            required
                            class="w-full px-4 py-3 border border-gray-600/50 bg-slate-700/50 text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-pink-500 focus:border-transparent transition-all duration-200"
                            placeholder="e.g. คณิตศาสตร์"
                        />
                    </div>
                     <div>
                        <label for="category_english_name" class="block text-sm font-medium text-gray-300 mb-2">Category Name (English)</label>
                        <input
                    ...
                    <div class="flex gap-3 pt-4">
                        <button
                            type="submit"
                            class="flex-1 bg-gradient-to-r from-pink-600 to-rose-600 text-white py-3 px-4 rounded-xl hover:from-pink-700 hover:to-rose-700 focus:outline-none focus:ring-2 focus:ring-pink-500 transition-all duration-200 font-medium shadow-lg hover:shadow-xl transform hover:-translate-y-0.5"
                        >
                            Save Category
                        </button>
                        <button
